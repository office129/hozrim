...
export function NewAdminModal({
  ...
  async function submit() {
    ...
    setLoading(true);
    try {
      const data = await apiSend("/api/admin/team", "POST", { name, email });
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-[380px] bg-card rounded-[18px] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="font-heading font-bold text-lg text-ink mb-4">מנהל/ת חדש/ה</div>
